...
dt %>% filter(donor == 'USA') %>%
  ...
  summarize(
    oda = median(total_oda)
  ) %>%
  ggplot() +
  aes(
    year, oda, color = as.factor(atopally)
  ) +
  geom_line()
dt %>% filter(donor == 'USA') %>%
  group_by(
  ...
  mutate(
    alliance = case_when(
      defense & !nonagg ~ 'Defense',
      !defense & nonagg ~ 'Nonaggression',
      defense & nonagg ~ 'Both',
      TRUE ~ 'None'
    )
  ) %>%
...
```{r}
dt %>% filter(donor == 'USA', defense == 1)%>%
  select(donor, recipient, year, defense:consul) %>%
  arrange(recipient)
```
